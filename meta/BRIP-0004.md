...
6. **Event Emission**: The enshrined function will emit the same events as the external function for consistency and observability
...
5. **Reward Distribution**: Process rewards for the proposing validator using the latest cutting board configuration, emitting distribution logs
...
8. **Event Emission**: Verify that enshrined `distributeFor` calls emit the same events as external calls
