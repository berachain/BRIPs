...
function distributeFor(
    ...
    bytes calldata previousProposerPubkey
    ...
    _executeDistribution(nextTimestamp, previousProposerPubkey);
}
```
...
- **Previous proposer pubkey source**: The 48-byte BLS public key will be passed from the consensus layer via the `BerachainPayloadAttributes.previous_proposer_pubkey` field
...
- **Operator Role**: A new operator role will be introduced that can update cutting board configurations without having access to reward claims or other validator privileges
- **Hot Key Separation**: Validators can delegate cutting board updates to a separate "hot key" (operator address) while keeping their reward-receiving "cold key" secure and offline
...
- **Security Benefits**: This separation reduces the attack surface by allowing frequent cutting board updates through a hot key while protecting the main validator rewards and high-privilege operations with a cold key
- **Operator Compromise Recovery**: In the event that an operator key is compromised, the validator can immediately revoke the operator's permissions using their cold key and designate a new operator
...
6. **Incentive Fulfillment**: Call enshrined `distributeFor` to fulfill incentives for the proposing validator using the latest cutting board configuration from step 4. The function is designed to never revert, but if an unexpected revert occurs, the protocol silently ignores it and continues processing
...
- **Implement cutting board operator separation**: Add operator delegation functionality to allow validators to designate separate hot keys for cutting board updates while maintaining cold key control over operator management and reward claims
...
1. **Normal Operation**: Verify automatic distribution occurs for each block with correct previous proposer pubkey
2. **Missing Pubkey**: Verify that forkchoice attributes without previous proposer pubkey result in invalid blocks after hard fork activation
3. **Invalid Pubkey**: Verify that malformed or invalid previous proposer public keys result in invalid blocks
...
13. **Block Validity**: Confirm that missing or invalid previous proposer pubkeys result in block rejection
...
- The consensus layer must be trusted to provide accurate previous proposer pubkeys
...
- Missing or invalid previous proposer pubkeys will result in block rejection, ensuring network consistency
...
- **[PR #26](https://github.com/berachain/bera-reth/pull/26)**: Foundation for `BerachainPayloadAttributes` with previous proposer pubkey support (execution layer)
