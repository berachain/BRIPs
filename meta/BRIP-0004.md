...
1. **Require** the `validator_pubkey` field in `BerachainPayloadAttributes` for all forkchoice updates after the hard fork activation
2. Extract the `validator_pubkey` from `BerachainPayloadAttributes` in forkchoice updates
...
**Note**: After the hard fork activation, forkchoice attributes that do not include the `validator_pubkey` field will be rejected by the execution layer to ensure consistent reward distribution.

### Execution Flow

1. **Block Start**: Execution layer receives forkchoice update with validator pubkey
2. **Transaction Processing**: Process all user transactions, including any validator cutting board updates
3. **Post-Transaction Processing**: After all transactions are processed, call enshrined `distributeFor`
4. **Reward Distribution**: Process rewards for the proposing validator using the latest cutting board configuration
...
2. **Missing Pubkey**: Verify that forkchoice attributes without validator pubkey are properly rejected after hard fork activation
