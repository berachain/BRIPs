...
The execution layer will automatically call the `distributeFor` function at the beginning of each block processing, before regular transaction execution. This requires modifications to the block execution process in forked execution clients.

### Modified Function Signature

The enshrined `distributeFor` function will use a simplified signature that leverages validator information available from forkchoice attributes:
...
Building on the foundation established in PR #26, the execution layer will:
...
The `Distributor.sol` contract will require updates to support the new simplified interface:

1. Add a new function variant that accepts only timestamp and validator pubkey
2. Maintain backward compatibility with the existing proof-based function for external calls
3. Implement access controls to ensure only the execution layer can call the simplified version
...
- **EIP-4788 (processBeaconBlockRoot)**: Automatically stores beacon block roots for cryptographic verification
- **EIP-4895 (processWithdrawalQueue)**: Handles validator withdrawals as part of block execution
...
- Gas costs for distribution transactions (estimated 200,000-500,000 gas per distribution)
...
- Modify `Distributor.sol` to support simplified enshrined interface
- Add access controls for execution layer calls
- Deploy and test updated contracts

### Phase 2: Execution Layer Integration
- Complete implementation of `BerachainEngineTypes` and custom payload handling
- Integrate `distributeFor` calls into block execution process
- Add comprehensive testing for reward distribution timing
...
### Access Control
- Only the execution layer should be able to call the simplified `distributeFor` function
...
- **PR #26**: Foundation for `BerachainPayloadAttributes` with validator pubkey support
- **Distributor.sol**: Current implementation of reward distribution logic at `/Users/rezbera/Code/contracts/src/pol/rewards/Distributor.sol`
- **Ethereum EIPs**: EIP-4788 and EIP-4895 as models for enshrined system operations
