...
7. **Migration Path**: Consider deprecating or disabling the external function after enshrined system proves stable to simplify the codebase and eliminate external bot dependency
...
### Phase 4: Migration (Future Consideration)
- After enshrined system proves stable and reliable, consider deprecating the external `distributeFor` function
- Options include: disabling external function, removing it entirely, or maintaining both permanently
- Would eliminate the need for external actors to call `distributeFor` and simplify the contract interface
- Requires careful consideration of ecosystem dependencies and migration timeline
...
8. **Event Emission**: Determine whether enshrined `distributeFor` calls can emit events and ensure consistent behavior with external calls
9. **Block Validity**: Confirm that missing or invalid proposer pubkeys result in block rejection
...
Once the enshrined `distributeFor` system proves stable and reliable, the external proof-based function could be deprecated or removed entirely. This would:
...
- Monitor ecosystem usage of external function before deprecation
- Provide sufficient notice period for any dependent systems
- Consider gradual approach: disable external function first, then remove in subsequent upgrade
